...
export default function MaterialCalculator({ onClose, onAddToBudget }) {
  ...
  const wasteValue = Math.min(100, Math.max(0, numberValue(waste)));

  const baseResult = useMemo(() => {
    if (material === 'wall') return l > 0 && h > 0 ? l * h : 0;
    // Never assume a concrete thickness: an implicit default can hide a costly input error.
    if (material === 'concrete') return l > 0 && w > 0 && h > 0 ? l * w * h : 0;
    return l > 0 && w > 0 ? l * w : 0;
  }, [material, l, w, h]);

  const reserve = 1 + wasteValue / 100;
  ...
  const blockBreakdown = useMemo(() => {
    ...
    const customArea = (numberValue(blockLength) / 100) * (numberValue(blockHeight) / 100);
  ...
  const paintBreakdown = useMemo(() => {
    ...
    const coverage = numberValue(paintCoverage);
    const coats = Math.min(10, Math.max(1, numberValue(paintCoats) || 1));
  ...
  const price = Math.max(0, numberValue(unitPrice));

  const estimatedTotal = useMemo(() => {
    if (!price || !displayQuantity) return 0;
    return priceMode === 'total' ? price : displayQuantity * price;
  }, [price, displayQuantity, priceMode]);

  const validationMessage = useMemo(() => {
    if (material === 'concrete' && (l <= 0 || w <= 0 || h <= 0)) return 'Para concreto debes indicar largo, ancho y espesor mayores que 0.';
    if (material === 'wall' && (l <= 0 || h <= 0)) return 'Para muro debes indicar largo y alto mayores que 0.';
    if (material !== 'wall' && material !== 'concrete' && (l <= 0 || w <= 0)) return 'Indica largo y ancho mayores que 0 para calcular.';
    if (material === 'block' && blockSize === 'custom' && (!numberValue(blockLength) || !numberValue(blockHeight))) return 'Indica las medidas de la pieza personalizada.';
    if (material === 'paint' && !numberValue(paintCoverage)) return 'Indica el rendimiento de pintura en m²/L.';
    return '';
  }, [material, l, w, h, blockSize, blockLength, blockHeight, paintCoverage]);

  function add() {
    if (!displayQuantity || validationMessage) return;
  ...
  return (
    <div className="modal-backdrop">
      <section className="project-form" aria-label="Calculadora de materiales">
        ...
        {validationMessage && <small role="alert" style={{ display: 'block', marginBottom: 12 }}>⚠️ {validationMessage}</small>}
        ...
        <div className="form-row"><button className="primary form-submit" type="button" disabled={!displayQuantity || Boolean(validationMessage)} onClick={add}>Añadir al presupuesto</button><button className="form-submit" type="button" onClick={onClose}>Listo</button></div>
      </section>
    </div>
  );
}
