export default function QuotePreview({ budget, project, client, onClose }) {
  ...
  const directCost = Number(budget.directSubtotal ?? budget.subtotal ?? 0);
  const indirectRate = Number(budget.indirectPercent ?? budget.indirectRate ?? 0);
  const indirectCost = Number(budget.indirectCosts ?? budget.indirectCost ?? directCost * indirectRate / 100);
  const costBase = Number(budget.costBase ?? directCost + indirectCost);
  const markupRate = Number(budget.markup ?? budget.margin ?? 0);
  const profit = Number(budget.profit ?? costBase * markupRate / 100);
  ...
  return <div className="modal-backdrop"><div className="quote-sheet" id="masterobrix-quote">
    ...
    <div className="quote-table"><div className="quote-row quote-head"><span>Descripción</span><span>Cant.</span><span>Precio</span><span>Total</span></div>{items.map((item,index)=><div className="quote-row" key={item.id || index}><span>{item.description}<small>{item.category} · {item.unit}</small></span><span>{item.quantity}</span><span>{money(item.unitPrice)}</span><strong>{money((Number(item.quantity||0)*Number(item.unitPrice||0)))}</strong></div>)}</div>
    <div className="quote-total"><div><span>Costo directo</span><strong>{money(directCost)}</strong></div><div><span>Costos indirectos ({indirectRate}%)</span><strong>{money(indirectCost)}</strong></div><div><span>Costo base</span><strong>{money(costBase)}</strong></div><div><span>Utilidad ({markupRate}%)</span><strong>{money(profit)}</strong></div><div className="grand"><span>TOTAL AL CLIENTE</span><strong>{money(total)}</strong></div></div>
