...
export default function ProjectDashboard({ project, budgets, expenses, materials, onClose, onOpenMaterials, onOpenExpenses, onOpenProfitability, onOpenBudget }) {
  ...
  return <div className="modal-backdrop"><section className="project-form project-dashboard" aria-label="Centro de control de obra">
    ...
    <div className="stats-grid">
      <div className="stat-card"><span>Presupuesto</span><strong>${planned.toLocaleString()}</strong></div>
      <div className="stat-card"><span>Gastado</span><strong>${spent.toLocaleString()}</strong></div>
      <div className="stat-card"><span>Disponible</span><strong>${remaining.toLocaleString()}</strong></div>
    ...
    <div className="action-grid">
      <button className="action-card" type="button" onClick={onOpenMaterials}><span className="action-icon">🧱</span><strong>Materiales</strong><small>Ver necesarios y comprados</small></button>
      <button className="action-card" type="button" onClick={onOpenExpenses}><span className="action-icon">💸</span><strong>Registrar gasto</strong><small>Guardar compra o gasto real</small></button>
      <button className="action-card" type="button" onClick={onOpenBudget}><span className="action-icon">💰</span><strong>Presupuesto</strong><small>Crear o revisar cotización</small></button>
      <button className="action-card" type="button" onClick={onOpenProfitability}><span className="action-icon">📈</span><strong>Rentabilidad</strong><small>Ver cuánto ganas</small></button>
    </div>
