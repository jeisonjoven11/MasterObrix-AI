...
export default function ProjectDashboard({ project, budgets, expenses, materials, onClose, onOpenMaterials, onOpenExpenses, onOpenProfitability, onOpenBudget }) {
  ...
  const alerts = useMemo(() => [
    ...
    ...(materialMissing > 0 ? [`🧱 Faltan ${materialMissing.toLocaleString('es-CO')} unidades de materiales según los registros.`] : []),
  ...
  const nextAction = planned <= 0
    ...
    : projectedCost > planned
      ...
      : materialMissing > 0
        ? `Revisa las ${materialMissing.toLocaleString('es-CO')} unidades pendientes y confirma sus precios antes de comprar.`
        : consumed >= 80
  ...
  return <div className="modal-backdrop"><section className="project-form project-dashboard" aria-label="Centro de control de obra">
    ...
    <div className="stats-grid">
      <div className="stat-card"><span>Presupuesto</span><strong>{money(planned)}</strong></div>
      <div className="stat-card"><span>Gastado</span><strong>{money(spent)}</strong></div>
      <div className="stat-card"><span>Disponible</span><strong className={remaining < 0 ? 'negative' : ''}>{money(remaining)}</strong></div>
      <div className="stat-card"><span>Consumido</span><strong>{progress}%</strong></div>
    </div>

    <div className="budget-summary">
      <div><span>Estado financiero</span><strong>{status}</strong></div>
      <div role="progressbar" aria-label="Porcentaje de presupuesto consumido" aria-valuenow={progress} aria-valuemin="0" aria-valuemax="100" style={{height:10,background:'var(--surface-2)',borderRadius:99,overflow:'hidden'}}>
        <div style={{height:'100%',width:`${progress}%`,background:'var(--accent)',borderRadius:99,transition:'width .25s ease'}} />
      </div>
      <small>{budgeted > 0 ? `${money(budgeted)} presupuestados en cotizaciones` : 'Todavía no hay presupuestos vinculados a esta obra.'}</small>
    </div>

    <div className="stats-grid">
      <div className="stat-card"><span>Materiales comprados</span><strong>{money(materialSpent)}</strong></div>
      <div className="stat-card"><span>Material pendiente</span><strong>{money(pendingMaterialCost)}</strong></div>
      <div className="stat-card"><span>Costo proyectado</span><strong>{money(projectedCost)}</strong></div>
      <div className="stat-card"><span>Saldo proyectado</span><strong className={projectedBalance < 0 ? 'negative' : ''}>{money(projectedBalance)}</strong></div>
    </div>

    <div className="budget-summary"><span>🤖 Recomendación MasterObrix</span><strong>{nextAction}</strong></div>

    {alerts.length > 0 ? <div className="empty-state" role="alert"><strong>⚠️ Atención</strong>{alerts.map(a => <p key={a}>{a}</p>)}</div> : <div className="empty-state"><strong>✅ Sin alertas</strong><p>La información registrada no muestra riesgos financieros en este momento.</p></div>}
