...
export default function ProjectMaterials({ projects, materials, initialProjectId='', onSave, onAddExpense, onClose }) {
  ...
  const categories=['Todas',...new Set(CATALOG.map(p=>p[2]))];
  const catalog=CATALOG.filter(p=>(category==='Todas'||p[2]===category)&&`${p[0]} ${p[2]}`.toLowerCase().includes(search.toLowerCase()));
  ...
  return <div className="modal-backdrop"><section className="project-form" aria-label="Materiales de obra">
    ...
    <div className="budget-summary"><div><span>Materiales controlados</span><strong>{rows.length}</strong></div><div><span>Comprado</span><strong>{totals.purchased}</strong></div><div><span>Gastado en materiales</span><strong>${totals.spent.toLocaleString()}</strong></div><div className="total"><span>Falta por comprar (estimado)</span><strong>${totals.pendingCost.toLocaleString()}</strong></div></div>
