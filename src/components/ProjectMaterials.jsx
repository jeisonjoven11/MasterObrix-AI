...
export default function ProjectMaterials({ projects, materials, onSave, onAddExpense, onClose }) {
  ...
  function registerPurchase(material){
    const amount=Number(window.prompt(`¿Cuánto pagaste por ${material.name}?`,String((Number(material.price)||0)*(Number(material.purchased)||1))));
    if(!Number.isFinite(amount)||amount<=0)return;
    onAddExpense({id:crypto.randomUUID(),projectId,description:`Compra: ${material.name}`,category:'Materiales',amount,date:new Date().toISOString().slice(0,10),createdAt:new Date().toISOString()});
    window.alert(`Compra registrada por $${amount.toLocaleString()}. El gasto ya cuenta en la obra.`);
  }
  return <div className="modal-backdrop"><section className="project-form" aria-label="Materiales de obra">
    ...
    {rows.length>0&&<div className="budget-summary">{rows.map(m=>{const missing=Math.max(0,Number(m.needed)-Number(m.purchased));return <div key={m.id}><span>{m.name}<br/><small>{missing?`Faltan ${missing} ${m.unit}`:'✓ Completo'}</small></span><strong>${(Number(m.purchased||0)*Number(m.price||0)).toLocaleString()} <button type="button" onClick={()=>registerPurchase(m)}>💸 Registrar compra</button></strong></div>})}<div className="total"><span>Compras estimadas</span><strong>${total.toLocaleString()}</strong></div></div>}
