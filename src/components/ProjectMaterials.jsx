...
export default function ProjectMaterials({ projects, materials, onSave, onAddExpense, onClose }) {
  ...
  const total=useMemo(()=>rows.reduce((s,m)=>s+(Number(m.purchased)||0)*(Number(m.price)||0),0),[rows]);
  const pending=useMemo(()=>rows.reduce((s,m)=>s+Math.max(0,Number(m.needed||0)-Number(m.purchased||0)),0),[rows]);
  ...
  function registerPurchase(m){const quantity=Number(m.purchased)||0;const unitPrice=Number(m.price)||0;if(!quantity||!unitPrice||!onAddExpense)return;onAddExpense({id:crypto.randomUUID(),projectId:m.projectId,description:`Compra de ${m.name}`,category:'Materiales',amount:quantity*unitPrice,date:new Date().toISOString().slice(0,10)});}
  return <div className="modal-backdrop"><section className="project-form" aria-label="Materiales de obra">
    ...
    <label>🛒 Buscar material<input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Cemento, arena, bloque..." /></label>
    {search&&<div className="empty-state">{catalog.map(p=><button className="form-submit" type="button" key={p[0]} onClick={()=>{setName(p[0]);setUnit(p[1]);setPrice(String(p[3]));setSearch('')}}>{p[0]} · {p[2]} · ${p[3].toLocaleString()} / {p[1]}</button>)}{!catalog.length&&<p>No encontramos ese material en el catálogo de referencia.</p>}<small>Precios orientativos: confirmar proveedor, ciudad y precio antes de comprar.</small></div>}
    ...
    {rows.length>0&&<div className="budget-summary"><div className="total"><span>Material pendiente</span><strong>{pending} unidades aprox.</strong></div>{rows.map(m=>{const missing=Math.max(0,Number(m.needed)-Number(m.purchased));const purchaseTotal=Number(m.purchased||0)*Number(m.price||0);return <div key={m.id}><span>{m.name}<small>{` ${m.purchased||0}/${m.needed} ${m.unit}`}</small></span><strong>{missing?`Faltan ${missing} ${m.unit}`:'✓ Completo'} · ${purchaseTotal.toLocaleString()} {purchaseTotal>0&&<button type="button" onClick={()=>registerPurchase(m)}>Registrar compra</button>}</strong></div>})}<div className="total"><span>Compras estimadas</span><strong>${total.toLocaleString()}</strong></div></div>}
