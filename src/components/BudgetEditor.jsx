...
export default function BudgetEditor({ projects, initialItem, initialProjectId, onSave, onClose }) {
  ...
  function submit(event) {
    ...
    onSave({ id: crypto.randomUUID(), projectId, items, margin: Number(margin) || 0, subtotal, profit, total, categoryTotals, createdAt: new Date().toISOString() });
  }
  return <div className="modal-backdrop"><form className="budget-form" onSubmit={submit}>
    ...
    <div className="budget-summary"><label>Margen de utilidad (%)<input type="number" min="0" step="0.5" value={margin} onChange={(e) => setMargin(e.target.value)} /></label><div><span>Materiales</span><strong>${categoryTotals.Materiales.toLocaleString()}</strong></div><div><span>Mano de obra</span><strong>${categoryTotals['Mano de obra'].toLocaleString()}</strong></div><div><span>Otros</span><strong>${categoryTotals.Otros.toLocaleString()}</strong></div><div><span>Costo base</span><strong>${subtotal.toLocaleString()}</strong></div><div><span>Utilidad</span><strong>${profit.toLocaleString()}</strong></div><div className="total"><span>Total al cliente</span><strong>${total.toLocaleString()}</strong></div></div>
